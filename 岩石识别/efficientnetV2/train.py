...
def main(args):
    ...
    data_transform = {
        "train": transforms.Compose([#transforms.RandomResizedCrop(img_size[num_model][0]), #随机缩放裁剪
                                     transforms.Resize((300,384)),
                                     transforms.RandomHorizontalFlip(), #随机水平翻转

                                     transforms.ToTensor(),  #transforms.ToTensor()函数的作用是将原始的PILImage格式或者numpy.array格式的数据格式化为可被pytorch快速处理的张量类型
                                     transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]), #Normalize()是对数据按通道进行标准化，即减去均值，再除以方差
